...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:TPLocalizedString(@"Pull to refresh",nil),
                             TPLocalizedString(@"Release to refresh",nil),
                             TPLocalizedString(@"Loading...", nil),
                                nil];
...
- (UIImageView *)arrow {
    if(!_arrow) {
        _arrow = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54-18, 20, 20)];
        CGPoint point= _arrow.center;
        point.x=self.bounds.size.width/2;
        _arrow.center=point;
        
        _arrow.backgroundColor = [UIColor clearColor];
        _arrow.image = [UIImage imageNamed:@"LLPullDown"],
