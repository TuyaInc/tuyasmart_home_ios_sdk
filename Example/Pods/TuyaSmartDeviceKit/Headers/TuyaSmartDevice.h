...
/**
 *  Get wifi signal strength. Receive signal strength from`-[TuyaSmartDeviceDelegate device:signalStrength:]`.
 *  获取wifi的信号强度。通过`-[TuyaSmartDeviceDelegate device:signalStrength:]`接收回调
 *
 *  @param success Success block
 *  @param failure Failure block
 */
 ...
 *  @param failure  Failure block
 */
- (void)publishMessageInLanWithBody:(NSDictionary *)body type:(NSInteger)type success:(TYSuccessHandler)success failure:(TYFailureError)failure;
