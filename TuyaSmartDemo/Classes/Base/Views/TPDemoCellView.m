...
+ (TPDemoCellView *)seperateCellView:(CGRect)frame  backgroundColor:(UIColor *)backgroundColor {
    TPDemoCellView *seperateCellView = [[TPDemoCellView alloc] initWithFrame:frame];
...
- (void)setDelegate:(NSObject<TPDemoCellViewDelegate> *)delegate {
    if (!_delegate) {
        [self addGestureRecognizer:[TPDemoViewUtil singleFingerClickRecognizer:self sel:@selector(viewCellTap)]];        
    }
    _delegate = delegate;
}
...
- (void)setRightItem:(TPDemoCellViewItem *)rightItem {
...
- (void)showRightArrow {
    if (!_rightItem) {
        _rightItem = [[TPDemoCellViewItem alloc] init];
    }
