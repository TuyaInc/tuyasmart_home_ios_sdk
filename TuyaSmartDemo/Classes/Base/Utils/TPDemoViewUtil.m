...
static const void *TYSDKDemoGestureRecognizerBlockKey = &TYSDKDemoGestureRecognizerBlockKey;

@implementation UIView (TYSDKDemoTap)

- (void)tysdkDemo_whenTapped:(void (^)(void))block {
    NSParameterAssert(block != nil);
    
    UITapGestureRecognizer *gesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(ty_didReviceTapGesture)];
    ...
    objc_setAssociatedObject(self, TYSDKDemoGestureRecognizerBlockKey, block, OBJC_ASSOCIATION_COPY_NONATOMIC);
    [self addGestureRecognizer:gesture];
}

- (void)ty_didReviceTapGesture {
    void (^block)(void) = objc_getAssociatedObject(self, TYSDKDemoGestureRecognizerBlockKey);
    
    NSLog(@"tap currnt class : %@", NSStringFromClass(self.class));
    if (block) block();
}
