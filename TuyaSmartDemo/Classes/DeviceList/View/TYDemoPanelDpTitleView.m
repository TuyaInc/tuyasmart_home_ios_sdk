...
@implementation TYDemoPanelDpTitleView
...
- (UILabel *)numLabel {
    if (!_numLabel) {
        _numLabel = [TPDemoViewUtil simpleLabel:CGRectMake(16, 17, 16, 16) f:10 tc:HEXCOLOR(0xFFFFFF) t:@""];
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        _titleLabel = [TPDemoViewUtil simpleLabel:CGRectMake(self.numLabel.right + 5, 15, 120, 19) f:17 tc:HEXCOLOR(0x303030) t:@""];
...
- (UILabel *)subTitleLabel {
    if (!_subTitleLabel) {
        _subTitleLabel = [TPDemoViewUtil simpleLabel:CGRectMake(15, self.numLabel.bottom, self.width - 15, 20) f:10 tc:HEXCOLOR(0x9B9B9B) t:@""];
    }
